...
DEFAULT_MANIFEST = REPO_ROOT / "manifests" / "artifact_bundles" / "role_contract_registry.bundle.json"
DEFAULT_SUBJECT = REPO_ROOT / "generated" / "agent_registry.min.json"
DEFAULT_BUNDLE_DIR = REPO_ROOT / "dist" / "abyss-artifact-bundle" / "aoa-agents-role-registry"
DEFAULT_REGISTRY_DIR = REPO_ROOT / "dist" / "abyss-artifact-registry" / "aoa-agents-role-registry"
DEFAULT_SUBJECT_STORE_ROOT = REPO_ROOT / "dist" / "abyss-artifact-subjects" / "aoa-agents-role-registry"
ARTIFACT_CLASS = "role_contract_registry"
CONSUMER_INTENT = "agent"
CONSUMER_REF = "aoa-agents:role-contract-registry"
SOURCE_REPO = "aoa-agents"
TRUST_ROOT_MODE = "host_managed"
PRODUCER = "aoa-agents build_published_surfaces role registry from source role profiles"
...
def _assert_manifest_matches_subject(manifest: Path, subject: Path) -> None:
    ...
    if payload.get("owner_repo") != "aoa-agents":
        raise ValueError("manifest owner_repo must be aoa-agents")
...
def _assert_public_safe_subjects(manifest: Path, subject: Path) -> None:
    ...
    if leaks:
        raise ValueError("role registry bundle subjects contain private or machine-local markers: " + ", ".join(leaks))
...
def _assert_expected_controls(artifact_bundles: Any, bundle_dir: Path, verify: dict[str, Any], identity: dict[str, Any]) -> None:
    ...
    if (bundle_dir / artifact_bundles.SBOM_CYCLONEDX_SIDECAR).exists():
        raise ValueError("role registry bundle must not emit SBOM before package/release-bundle status")
...
def _trust_gate_allow_latest(
    ...
    *,
    ...
    common_claims_ok = bool(
        decision.get("model") == "fail_closed_consumer_admission"
    ...
    admitted = bool(
        ...
        and common_claims_ok
        and (
            not require_subject_store
            or inspected_claims.get("artifact_subject_store", {}).get("ok") is True
        )
    )
    expected_pre_materialization_deny = bool(
        not require_subject_store
        and trust_gate.get("ok") is False
        and trust_gate.get("verdict") == "deny"
        and decision.get("allow") is False
        and common_claims_ok
        and trust_gate.get("blockers") == ["required_artifact_subject_store_not_verified"]
        and trust_gate.get("reasons") == ["required_artifact_subject_store_not_verified"]
        and inspected_claims.get("artifact_subject_store", {}).get("ok") is False
    )
    return {
        "ok": admitted or expected_pre_materialization_deny,
        "posture": (
            "already_materialized_allow"
            if admitted
            else "expected_pre_materialization_deny"
            if expected_pre_materialization_deny
            else "invalid"
        ),
...
def _verify_private_registry_marker(manifest: Path, subject: Path, tmp_root: Path) -> dict[str, Any]:
    candidate = tmp_root / "private-agent-registry.min.json"
    shutil.copyfile(subject, candidate)
    candidate.write_text(candidate.read_text(encoding="utf-8") + "\nTOKEN=private-negative\n", encoding="utf-8")
    try:
        _assert_public_safe_subjects(manifest, candidate)
    except ValueError as exc:
        return {"ok": "role registry bundle subjects contain private or machine-local markers" in str(exc), "error": str(exc)}
    return {"ok": False, "error": "private role registry marker was not detected"}
...
def _verify_terminal_registry_state(
    ...
    revoked = artifact_bundles.write_bundle_registry_record(
        ...
        revocation_reason="aoa-agents role registry terminal-state rehearsal",
        source_repo=SOURCE_REPO,
        source_ref=_portable_ref(manifest),
        producer=PRODUCER,
        trust_root_mode=TRUST_ROOT_MODE,
...
def _run_adversarial_checks(
    ...
    with tempfile.TemporaryDirectory(prefix="aoa-agents-role-registry-negative-", dir=_default_tmp_root()) as tmp:
        tmp_root = Path(tmp)
        checks = {
...
def _validate_in_bundle_dir(
    ...
    *,
    ...
    producer_command = "python scripts/validate_abyss_machine_role_registry_bundle.py"
    ...
    payload = {
        ...
        "schema": "aoa_agents_abyss_machine_role_registry_artifact_bundle_validation_v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate aoa-agents role registry through OS Abyss artifact bundles.")
    ...
    if args.json:
        ...
        print(
            "[ok] abyss-machine aoa-agents role registry artifact bundle verified: "
