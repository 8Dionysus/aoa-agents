...
def validate_antifragility_stress_surfaces() -> None:
    ...
    if not (REPO_ROOT / "agents" / "roles" / "architect" / "profile.json").is_file():
        fail("agents/roles/architect/profile.json must exist for the additive stress posture example")
...
def validate_agent_profile_sources() -> list[dict[str, object]]:
    ...
    if not profiles:
        fail("agents/roles/ must contain at least one '*/profile.json' file")
    ...
    if actual_registry != expected_registry:
        fail("generated/agent_registry.min.json drifted from agents/roles/*/profile.json")
    ...
    for index, payload in enumerate(profiles):
        ...
        expected_profile_path = PROFILES_DIR / profile_name / "profile.json"
...
def validate_model_tier_sources() -> list[dict[str, object]]:
    ...
    if not tiers:
        fail("agents/operating-model/tiers/ must contain at least one '*.tier.json' file")

    expected_registry = build_model_tier_registry_payload(tiers)
    actual_registry = read_json(MODEL_TIER_REGISTRY_PATH)
    if actual_registry != expected_registry:
        fail("generated/model_tier_registry.json drifted from agents/operating-model/tiers/*.tier.json")
...
def validate_orchestrator_class_sources() -> list[dict[str, object]]:
    ...
    if not classes:
        fail("agents/operating-model/orchestrators/ must contain at least one '*.class.json' file")
    ...
    if actual_catalog != expected_catalog:
        fail("generated/orchestrator_class_catalog.min.json drifted from agents/operating-model/orchestrators/*.class.json")
    ...
    if actual_capsules != expected_capsules:
        fail("generated/orchestrator_class_capsules.json drifted from agents/operating-model/orchestrators/*.class.json")
    ...
    if actual_sections != expected_sections:
        fail("generated/orchestrator_class_sections.full.json drifted from agents/operating-model/orchestrators/*.class.json")
...
def validate_cohort_pattern_sources() -> list[dict[str, object]]:
    ...
    if not patterns:
        fail("agents/operating-model/cohorts/ must contain at least one '*.pattern.json' file")

    expected_registry = build_cohort_registry_payload(patterns)
    actual_registry = read_json(COHORT_COMPOSITION_REGISTRY_PATH)
    if actual_registry != expected_registry:
        fail("generated/cohort_composition_registry.json drifted from agents/operating-model/cohorts/*.pattern.json")
...
def validate_runtime_seam_binding_sources() -> list[dict[str, object]]:
    ...
    if not bindings:
        fail("agents/operating-model/runtime-seams/ must contain at least one '*.binding.json' file")

    expected_registry = build_runtime_seam_registry_payload(bindings)
    actual_registry = read_json(RUNTIME_SEAM_BINDINGS_PATH)
    if actual_registry != expected_registry:
        fail("generated/runtime_seam_bindings.json drifted from agents/operating-model/runtime-seams/*.binding.json")
...
def validate_orchestrator_class_catalog() -> dict[str, dict[str, object]]:
    ...
    for index, entry in enumerate(orchestrator_classes):
        ...
        expected_source_path = f"agents/operating-model/orchestrators/{class_id}.class.json"
