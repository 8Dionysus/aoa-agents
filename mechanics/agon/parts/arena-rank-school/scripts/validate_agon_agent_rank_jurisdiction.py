...
BUILDER = Path(__file__).resolve().parent / "build_agon_agent_rank_jurisdiction_registry.py"
...
def expected_registry() -> dict:
    spec = importlib.util.spec_from_file_location("agon_agent_rank_builder", BUILDER)
...
def main() -> int:
    ...
    if data.get("live_protocol") is not EXPECTED_LIVE_PROTOCOL:
        raise SystemExit("rank jurisdiction registries must remain non-live protocol surfaces")
