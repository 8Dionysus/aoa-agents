# Adoption Retention Schemas

Part-local schemas for Agon adoption and retention contracts.

| Schema | Contract |
| --- | --- |
| [agent-kind-adoption-boundary.schema.json](agent-kind-adoption-boundary.schema.json) | kind-safe adoption boundary |
| [agonic-adoption-scar-candidate.schema.json](agonic-adoption-scar-candidate.schema.json) | candidate scar handoff |
| [agonic-pattern-adoption-trial.schema.json](agonic-pattern-adoption-trial.schema.json) | pattern adoption trial |
| [agonic-pattern-retention-obligation.schema.json](agonic-pattern-retention-obligation.schema.json) | retention obligation |
| [agonic-shared-scar-harvest.schema.json](agonic-shared-scar-harvest.schema.json) | shared scar harvest |

Validate with:

```bash
python scripts/validate_adoption_boundary_contracts.py
```
