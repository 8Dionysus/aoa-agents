...
Source graph hash: `55e1c41bd3bdc2c2264f119b7f45e8c4b75842e9eeed05154a28cace515e1d65`
Federation: `aoa-agents` specializes `aoa-skills:operations`.

| skill | visibility | use when | do not use when | version | fingerprint |
|---|---|---|---|---|---|
| `aoa-agents-skills` | advertised | The owning routing control plane or current holder presents an agent-tool decision whose responsibility boundary is not yet classified.; A request or plan exposes an agent, worker, reviewer, researcher, parallel lane, background role, or delegated duty whose responsibility boundary is not yet classified.; Goal planning, execution, or closeout exposes a duty that may need an independently addressable responsibility bearer.; An existing persistent role may need rebinding, return filtering, or controlled wake. | No agent-tool decision, delegation pressure, or independent-duty pressure exists. | 0.5.0 | `b90df838d237de2b` |
