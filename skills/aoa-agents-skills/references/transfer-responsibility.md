...
actor mandate, incarnation binding, task-local DAG node, current holder,
...
4. Use `aoa-summon` only through an external CLI lane that consumes the exact
   incarnation binding, canonical SDK summon request/decision, and runtime
   launch refs, and returns a real separate process/session handle. Never
